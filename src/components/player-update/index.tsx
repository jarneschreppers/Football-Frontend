...
const UpdatePlayer: React.FC = () => {
    ...
    const updatePlayer = async (name: string, birthplace: string, birthdate: string, position: string) => {
        try {
            await  PlayerService.updatePlayer(name, birthplace ? birthplace : "", birthdate ? birthdate : "", position ? position : "");
            setStatusMessages([
                { message: `${name} successfully updated.`, type: 'success' },
            ]);
            setNameInput("name");
        } catch (error: any) {
            setStatusMessages([
                ...statusMessages,
                { message: "No player found with this name", type: 'error' },
            ]);
        }
    };
    ...
    const handleSubmit = (event: any) => {
        ...
        if (nameInput.trim() === '') {
            ...
            updatePlayer(nameInput, birthplaceInput, birthdateInput, positionInput);
        }
    };
    ...
    return (
        ...
        <section className="row justify-content-center">
            <p className="row justify-content-center">Update a player</p>
            {statusMessages && (
